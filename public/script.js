...
const mq_data = {
  ...
  "Couenubia's Awakening": 234000000,
};
...
const evaluateDiaries = function () {
  ...
  if (mqBegin <= mqEnd) {
    ...
    for (let i = mqBegin; i <= mqEnd; i++) {
      ...
      if (i == 85 && !skipVenena) {
        // 85 = metacoenubia quest, not skipping gives +12.5m xp after decel fight
        mqXP += 12500000;
      }
    } // 2
    ...
    if (!exact_runs) {
      ...
      let curXP =
        getTotalXP(initialLv, initialLvP, lv) + (lvP / 100) * getXP(lv + 1);
      let stackedXP = 0;
      for (let i = mqBegin; i <= mqEnd; i++) {
